...
class MolBertFeaturizer:
    ...
    def __init__(
        ...
        try:
            ...
            raise FileNotFoundError(f"Could not find hparams.yaml in model directory: {self.model_dir}")
        # with open(self.hparams_path) as yaml_file:
        #    config_dict = yaml.load(yaml_file, Loader=yaml.FullLoader)
        ...
        checkpoint = torch.load(self.checkpoint_path, map_location=lambda storage, loc: storage, weights_only=False)
    ...
    def transform(self, molecules: Sequence[Any]) -> Tuple[Union[Dict, np.ndarray], np.ndarray]:
        ...
        with torch.no_grad():
            outputs = self.model.model.bert(input_ids=input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask)

        if self.output_all:
            sequence_output, pooled_output, hidden = outputs.last_hidden_state, outputs.pooler_output, outputs.hidden_states
        else:
            sequence_output, pooled_output = outputs.last_hidden_state, outputs.pooler_output

        # set invalid outputs to 0s
        valid_tensor = torch.tensor(valid, dtype=sequence_output.dtype, device=sequence_output.device, requires_grad=False)
